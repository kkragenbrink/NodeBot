...
var Mud = function() {
    ...
    self.login = function(data) {
        ...
        if (/"connect <name> <password>"/.test(data)) {
            NodeBot.log('Mud', "Logging in to %s.", NodeBot.config.mud.username);
            send("connect %s %s", NodeBot.config.mud.username, NodeBot.config.mud.password);
    ...
    function makeProcess(pid, data) {
        ...
        return NodeBot.Util.format('think u(TOJSON,type:process,pid:%d,data:[%s])', pid, data);
    }
