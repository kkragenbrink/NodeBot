...
var Mud = function() {
    ...
    self.login = function( data ) {
        ...
        if ( /"connect <name> <password>"/.test( data ) ) {
            ...
            send( '&COMMAND me=$' + NodeBot.config.cmdPrefix + '([^\\s/]+)(/\\S*)?\\s?(.*)?:think ifelse(gt(conn(%!),-1),u(TOJSON,type:command,requester:%#,command:%1,switches:%2,args:%3),pemit(%#,NodeBot is offline.))' );
